...
class HomeController extends GetxController with WidgetsBindingObserver {
  ...
  StreamSubscription<InterruptionState>? _interruptionStateSubscription;
  ...
  void _setupInterruptionManager() {
    ...
    _interruptionStateSubscription = manager.stateChanges.listen((state) {
      isInterrupted.value =
          state == InterruptionState.interrupted ||
          state == InterruptionState.pausedByInterruption;
    });
  }
  ...
  Future<void> _autoSaveInterruptedSession() async {
    try {
      ...
      await savePendingSession(defaultSessionTitle);
    } catch (error, stackTrace) {
  ...
  Future<void> startCaptioning() async {
    ...
    try {
      ...
      _captioningStartedAt = DateTime.now();
      _startDurationTimer();
      if (_interruptionManager != null) {
        unawaited(_interruptionManager.onSessionStarted().catchError(
          (error, stackTrace) {
            debugPrint(
                '[Transcribe] Interruption manager failed to start: $error');
          },
        ));
      }
  ...
  void onClose() {
    ...
    _interruptionStateSubscription?.cancel();
    _interruptionManager?.onSessionEnded();
    unawaited(ForegroundServiceHandler.stop());
    unawaited(_tearDown());
    super.onClose();
