...
| Android: CMake 3.19+ / `SPIRV-Headers` (`ggml-vulkan`) errors | Same script (disables Vulkan, uses CPU); install CMake 3.22+ via Android SDK Manager if prompted |
| iOS: whisper_kit compile / linker errors | `bash tool/patch_whisper_kit_ios.sh`, then `cd ios && pod install` |
| iOS: undefined symbol `_ggml_*` / duplicate `WhisperKitPlugin` | Same whisper_kit patch after `pub get` |
...
| ANR / UI freeze during ML | Rebuild with latest code; heavy work stays off UI thread |
